  ...
  module Geometry
    module WyeBuilder
      ...
      def self.add_round_wye_hub(group:, center:, forward_axis:, side_axis:, diameter:)
        ...
        south_pole = center.offset(normal_axis.clone.reverse, radius_normal)
        north_pole = center.offset(normal_axis, radius_normal)
        rings = []

        # The old implementation built full SEGMENTS-point rings at phi = +/- PI/2.
        # Those rings collapse to one point, producing degenerate faces such as
        # [pole, pole, ring_point, ring_point]. Keep the poles as single vertices
        # and only build real rings where cos(phi) is non-zero.
        (1...ROUND_HUB_RINGS).each do |ring_index|
          ...
          ring_center = center.offset(normal_axis, normal_component)
          ring = []
          ...
          ROUND_HUB_SEGMENTS.times do |segment_index|
            ...
            ring << Geom::Point3d.new(
              ring_center.x + forward_axis.x * forward_component + side_axis.x * side_component,
              ring_center.y + forward_axis.y * forward_component + side_axis.y * side_component,
              ring_center.z + forward_axis.z * forward_component + side_axis.z * side_component
            )
          end
        ...
        first_ring = rings.first
        ROUND_HUB_SEGMENTS.times do |segment_index|
          next_index = (segment_index + 1) % ROUND_HUB_SEGMENTS
          Mesh.add_face_safe(
            entities,
            [south_pole, first_ring[next_index], first_ring[segment_index]]
          )
        end

        (rings.length - 1).times do |ring_index|
          ...
          ROUND_HUB_SEGMENTS.times do |segment_index|
            ...
            # Ellipsoid cells are inherently slightly non-planar. Asking
            # SketchUp to create them as quads lets some cells remain quads while
            # others are internally/fallback triangulated, which produces uneven
            # smooth shading. Triangulate every cell the same way instead.
            Mesh.add_face_safe(
              entities,
              [
        ...
        last_ring = rings.last
        ROUND_HUB_SEGMENTS.times do |segment_index|
          next_index = (segment_index + 1) % ROUND_HUB_SEGMENTS
          Mesh.add_face_safe(
            entities,
            [last_ring[segment_index], last_ring[next_index], north_pole]
          )
        end
