  ...
  module Catalog
    ...
    module MasterFlowGeometry
      ...
      def build_round_wye(group:, layout:, product:)
        ...
        # Fabricated saddle/neck.  The buried end is slightly wider than the
        # branch tube so the visible intersection resembles the pressed lateral
        # seam in the product photo without introducing the generic ellipsoid hub.
        penetration = [outlet * 0.38, 0.55].max
        neck_length = [[outlet * 0.55, branch_length * 0.42].min, outlet * 0.30].max
        branch_inside = junction.offset(branch_axis.clone.reverse, penetration)
        neck_end = junction.offset(branch_axis, neck_length)
        ...
        if neck_end.distance(branch_end) > EPSILON
          Geometry::PipeBuilder.build_into(
            group, neck_end, branch_end, outlet,
            overlap_start: false, overlap_end: false,
            cap_start: false, cap_end: false
          )
        end
        ...
        add_round_seam(group.entities, neck_end, branch_end, branch_axis, outlet_radius)
        add_branch_attachment_seam(group.entities, junction, branch_axis, outlet_radius)
