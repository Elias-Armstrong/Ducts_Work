  ...
  module Services
    class EndWyeInsertService
      ...
      def self.insert_at_port(model:, network:, stem_port:, side_vector:)
        ...
        native_branch_dimensions =
          if mixed_side_takeoff
            Model::DuctDimensions.coerce(main_dimensions)
          else
            BranchTransitionService.socket_dimensions(
              main_dimensions: main_dimensions,
              requested_dimensions: requested_branch_dimensions
            )
          end
        ...
        ModelOperation.run(
          ...
          EndFittingSupport.finalize_end_fitting!(
            ...
            outlet_ports: [forward_port, native_branch_port]
          )
